...
import org.joyrest.oauth2.configurer.client.JdbcClientDetailsServiceConfigurer;
import org.joyrest.oauth2.configurer.user.JdbcUserDetailsServiceConfigurer;
...
public class JdbcOAuthBinder extends AbstractBinder {

    private AuthorizationServerConfiguration authorizationServerConfiguration() throws Exception {
        // @formatter:off
		final HikariDataSource dataSource = new HikariDataSource(new HikariConfig("/datasource.properties"));

	    final ClientDetailsService clientDetailsService = new JdbcClientDetailsServiceConfigurer(dataSource)
        ...
        final UserDetailsService userDetailsService = new JdbcUserDetailsServiceConfigurer(dataSource)
		    .withUser("roy", "spring")
				.roles("USER")
	        .and()
